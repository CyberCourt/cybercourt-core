...
contract TokenFaucet is OwnableUpgradeable {
  ...
  function unclaimedBalanceOf(address user) external view returns (uint256) {
    uint256 currentTimestamp = _currentTime();
    ...
    uint256 _exchangeRateMantissa= nextExchangeRateMantissa.toUint112();

    
    UserState storage userState = userStates[user];
    if (_exchangeRateMantissa == userState.lastExchangeRateMantissa) {
      // ignore if exchange rate is same
      return uint256(userState.balance);
    }
    uint256 deltaExchangeRateMantissa = uint256(_exchangeRateMantissa).sub(userState.lastExchangeRateMantissa);
    uint256 userMeasureBalance = measure.balanceOf(user);
    uint256 newUserTokens = FixedPoint.multiplyUintByMantissa(userMeasureBalance, deltaExchangeRateMantissa).toUint128();

    return uint256(userState.balance).add(newUserTokens);
  }
